...
DEFAULT_CONFIG = {
    ...
    "default_limit": 100,
    "default_lang": "en",
    "bot_token": "8511711035:AAG8ddIaqW2kQ8BAXJfZLssED64FxDppksE",          # <-- SET YOUR BOT TOKEN HERE (or via env var)
    "admin_ids": [7655738256]
}
...
def home():
    return jsonify({
        ...
        "endpoints": {
            "GET/POST /api/search": "Perform search",
...
def search():
    ...
    try:
        ...
        # Batch mode: list -> newline separated
...
if not bot_token:
    logger.warning("No bot token set. Telegram bot will not work.")
    bot = None
else:
    bot = telebot.TeleBot(bot_token)

def perform_search(query, limit, lang):
    """Call leakosint API and return JSON."""
    payload = {
        "token": config["leakosint_token"],
        "request": query,
        "limit": limit,
...
def format_results(result, query):
    """Format search results for Telegram message (Markdown)."""
    if "error" in result:
    ...
    for db_name, db_data in dbs.items():
        ...
        if "Data" in db_data and db_data["Data"]:
            for entry in db_data["Data"][:3]:  # first 3 entries
...
def cmd_status(message):
    ...
    text = f"📊 *API Status*\n"
    text += f"• Public API: `{'✅ Enabled' if config['api_enabled'] else '❌ Disabled'}`\n"
    text += f"• Default limit: `{config['default_limit']}`\n"
    text += f"• Default language: `{config['default_lang']}`\n"
    if admin:
        token = config["leakosint_token"]
        masked = token[:6] + "..." + token[-4:] if len(token) > 10 else "***"
        text += f"• Leakosint token: `{masked}`\n"
    bot.reply_to(message, text, parse_mode="Markdown")
...
def cmd_search(message):
    ...
    if len(parts) < 2:
        ...
        return
    query = parts[1].strip()
    # If API disabled, only admins can search
    if not config["api_enabled"] and not is_admin(message.from_user.id):
...
def cmd_balance(message):
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "⛔ Admin only command.")
        return
    # Simple test to see if token is valid
    try:
        test_payload = {
            "token": config["leakosint_token"],
            "request": "test",
        ...
        if resp.status_code == 200:
            ...
            bot.reply_to(message, f"❌ API returned status {resp.status_code}. Token may be invalid.")
    except Exception as e:
...
def webhook():
    if request.headers.get('content-type') == 'application/json' and bot:
...
def set_webhook():
    """Set the webhook for the bot using the Render public URL."""
    if not bot:
        return
    # Render provides the public URL via environment variable RENDER_EXTERNAL_URL
    public_url = os.environ.get("RENDER_EXTERNAL_URL")
    if not public_url:
        # For local testing, fallback to polling (but we don't want to rely on it)
        logger.warning("No RENDER_EXTERNAL_URL. Falling back to polling (unstable).")
...
if __name__ == "__main__":
    ...
    if not bot_token:
        logger.error("Bot token missing! Please edit config.json and add 'bot_token'.")
    else:
        set_webhook()
